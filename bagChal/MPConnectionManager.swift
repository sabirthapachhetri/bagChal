        ...
        if !session.connectedPeers.isEmpty {
            do {
                if let data = gameMove.data() {
                    ...
                    print("Data sent successfully: \(gameMove)")
                }
            } catch {
...
extension MPConnectionManager: MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        ...
        if let gameMove = try? JSONDecoder().decode(MPGameMove.self, from: data) {
            DispatchQueue.main.async {
                print("Decoded game move: \(gameMove)")
                ...
                case .start:
                    ...
                    break
                case .move:
                    if let receivedMove = gameMove.move {
                        self.game?.processReceivedMove(receivedMove)
                    }
                case .reset:
                    self.game?.resetGame()
                case .end:
